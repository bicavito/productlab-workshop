...
**Layout:**
[Describe the spatial arrangement of information on screen. Use a 2D mental model.
What is in the top area? What is in the left column? What is in the right column? What is at the bottom?
The goal is to fit all critical information within one viewport (no scrolling for primary content).
Derive the layout from Priority 1 — let the signal dictate the structure, not the other way around.
⚠ Do not copy the example below. Use it only to understand the expected format.
> Example format (do not use this layout): "Top: compact KPI strip (3 metrics in one row). Body: two columns — left shows X, right shows Y. Bottom: compact Z strip."]

**Priority 1 — The signal** (must be visible immediately, highest information value):
[What is the one thing that changes a decision? The alert, the outlier, the number that matters most. Describe the exact data condition, what makes it urgent, and how it must be displayed — as a highlighted count, a top-positioned list, a badge, etc. This should take the most visual weight in the layout.]

**Priority 2 — The context** (the body of the view, what the user spends time with):
[What is the main content that gives meaning to the signal? The breakdown, the distribution, the list. Describe exactly what fields are visible and how they are grouped. This is the "body" of the layout.]

**Priority 3 — Supporting data** (reference information, lower visual weight):
[What additional data does the user sometimes need, but does not need to see every time? This can be compressed, smaller, positioned below or alongside with lower visual weight.]
...
- Be accessible via a **tab** within the same feature workspace
...
- Supporting Views are navigated via tabs. Each tab must have a short label (1-2 words).
